...
export async function fetchAlbums(query: string): Promise<Album[]> {
  ...
  const offset = Math.floor(Math.random() * 500); // até 500 para variar
  const response = await fetch(
    `https://api.spotify.com/v1/search?q=${encodeURIComponent(query)}&type=album&limit=50&offset=${offset}`,
    {
